...
Codex 以及 OpenCode 配 `"npm": "@ai-sdk/openai"` 时走 `POST /v1/responses`（`input`、`instructions`、`function_call` / `function_call_output`、带 event 类型的 SSE）。`"npm": "@ai-sdk/openai-compatible"` 仍走 Chat Completions。Responses 会收下并忽略采样参数（`temperature`、`top_p` 等）；Chat Completions 对这些字段仍 400。`previous_response_id` 续同一个 Cursor Agent。
